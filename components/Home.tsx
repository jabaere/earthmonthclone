...
import { handleConnect } from "@/store/playerReducer";

export default function HomeScreen() {
  const dispatch = useDispatch();
  const player = useSelector((state: any) => state.player);
  const isConnect = player.isConnect;

  const variants = {
    visible: (custom: { enter: number; leave: number }) => ({
      opacity: 1,
      transition: {
        duration: custom.enter * 2.9,
        type: "spring",
        ease: "easeIn",
      },
      y: 0,
    }),
    exit: (custom: { enter: number; leave: number }) => ({
      opacity: 0,
      y: 50,
      transition: {
        duration: custom.leave,
        velocity: custom.leave,
  ...
  return (
    <div className={styles.container}>
      <div className={styles.left_side}>
        ...
        >
          <div className={styles.right_container}>
            ...
            <Image
              ...
              width={950}
              height={50}
              className={styles.right_earthgangpng}
              alt="image"
            />
          </div>
        </motion.div>
        ...
        {isConnect ? (
          <motion.div
            className={styles.main_button_container}
            custom={{ enter: 1, leave: 3.5 }}
            animate="visible"
            variants={variants}
          ...
          >
            <button className={styles.main_button}>
              ...
              <span className={styles.button_desktop_txt}>
                Connect your Spotify account to get started
              </span>
              <span className={styles.button_mobile_txt}>Connect Spotify</span>
            </button>
            ...
            <button
              ...
              onClick={() => dispatch(handleConnect(false))}
            >
          ...
          <div className={styles.spotify_status_container}>
            <div className={styles.player_container}>
              <iframe
                src="https://open.spotify.com/embed/playlist/37i9dQZF1DZ06evO3p37CU?theme=1"
                style={{ border: "none" }}
                id={styles.iframe}
              ></iframe>
            </div>
